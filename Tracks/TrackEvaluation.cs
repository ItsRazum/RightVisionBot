...
namespace RightVisionBot.Tracks
{
    ...
    class TrackEvaluation
    {
        public static InlineKeyboardMarkup RatingSystem(long userId)
        {
            ...
            new[]
            {
                new[]
                {
                    ...
                    InlineKeyboardButton.WithCallbackData(Count(userId), "count"),
        ...
        private static InlineKeyboardButton[] secondaryActions(long userId)
        {
            ...
            {
                InlineKeyboardButton.WithCallbackData("Сменить прошлую оценку", "back"),
        ...
        public static void Start(ITelegramBotClient botClient, Message message)
        {
            ...
            Program.updateLocation(userId, "evaluation");
        ...
        public static void First(ITelegramBotClient botClient, Message message)
        {
            ...
            { ResizeKeyboard = true };
            long userId = message.From.Id;
            var artistId = database.Read($"SELECT `userId` FROM `RV_C{CriticRoot.GetCritic(userId).Status}` WHERE `status` = 'ok' AND `userId` != {userId} AND `{userId}` = -1 LIMIT 1", "userId");
