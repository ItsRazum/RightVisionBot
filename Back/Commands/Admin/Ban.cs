...
class Restriction
{
    // Лист задействуется при очистке группы
    ...
    public static async Task Mute(ITelegramBotClient botClient, RvUser rvUser, Message message)
    {
        if (rvUser.Has(Permission.Mute))
        {
            ...
            if (mutedMember.Status is ChatMemberStatus.Member or ChatMemberStatus.Restricted)
            {
                ...
                RvUser.Get(mutedId).Punishments.Add(new(RvPunishment.PunishmentType.Mute, message.Chat.Id, "не указана", DateTime.Now, time));
    ...
    public static async Task Ban(ITelegramBotClient botClient, RvUser rvUser, Message message)
    {
        if (rvUser.Has(Permission.Ban))
        {
            ...
            if (bannedMember.Status is ChatMemberStatus.Member)
            {
                ...
                RvUser.Get(bannedUser.Id).Punishments.Add(new(RvPunishment.PunishmentType.Ban, message.Chat.Id, "не указана", DateTime.Now, DateTime.Now.AddDays(7)));
